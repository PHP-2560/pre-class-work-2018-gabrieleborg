...
chd.type<-as.vector(unique(wcgs["typchd69"]))
average_wgt<-data.frame(matrix(ncol = 1, nrow = length(chd.type$typchd69)))
rownames(average_wgt)<-chd.type
try<-data.frame()
for (i in chd.type) {
  sub_sample <- wcgs[wcgs$typchd69==i, "weight"]
  chd.type[i,1] <- mean(sub_sample, na.rm = TRUE)
  #print(paste0(chd.type[i]), " has mean ", sub_mean)
}
```
